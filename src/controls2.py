...
def StartTime(TRNData):
    ...
    thx_load_in  = TRNData[thisModule]["inputs"][20]
...
def dhw_set_increase_when_sun(demand, ctrIRR, coll_pump, tset_dhw):
    if not(demand) and ctrIRR:
        tset_dhw = 70
    elif demand and not(ctrIRR) and coll_pump:
        tset_dhw = 60
    elif demand and not(ctrIRR) and not(coll_pump):
        tset_dhw = 50
    elif tset_dhw <=-9000:
        tset_dhw = 40
    else:
...
def cal_div_load(ctrSHbuff, dhw_demand):
...
def cal_controls_no_ss_buff(demand, op_window, ctrIRR, coll_pump, tset_dhw):   
    ctr_hp = demand and  coll_pump and op_window
    load_hx_bypass = ctr_hp
    
    coll_pump2 = coll_pump  
    if coll_pump and not(demand):
        coll_pump2 = 0
    ...
    return ctr_hp, load_hx_bypass, tset_dhw, coll_pump2

def cal_controls_with_ss_buff(demand, op_window, ctrIRR, coll_pump, tset_dhw, ss_stat):
    ctr_hp = demand and op_window
    load_hx_bypass = ctr_hp
    
    coll_pump2 = coll_pump
    if coll_pump and ctr_hp and not(ss_stat): # ss_stat = hx_bypass
        coll_pump2 =0 
...
def Iteration(TRNData):
    ...
    if operation_mode ==4:
        ...
        t_inlet_below_ambient = tcoll_in <= tamb
    coll_pump = ctrIRR  or t_inlet_below_ambient or ctr_coll_t
    # coll_pump = 0.8 if ctrIRR else 1 if -25<tcoll_in<=tamb or ctr_coll_t else 0
    
    demand = ctrDHW or ctrSHbuff
    div_load = cal_div_load(ctrSHbuff, dhw_demand)
    ...
    if control_case!='with ss buff':
        ...
        op_window = (tcoll_out> -24) and (tcoll_out<35) #-24 < tcoll_out < 35
        ctr_hp, load_hx_bypass, tset_dhw, coll_pump2 = cal_controls_no_ss_buff(demand, op_window, ctrIRR, coll_pump, tset_dhw)
    else:
        hx_bypass = ss_stat
        op_window = -25<tss_top<35
        ctr_hp, load_hx_bypass, tset_dhw, coll_pump2 = cal_controls_with_ss_buff(demand, op_window, ctrIRR, coll_pump, tset_dhw, ss_stat)
    ...
    aux_set=50
    if thx_load_in>tcoll_out and hx_bypass==0 and coll_pump2==1:
        coll_pump2=0
        if demand:
            aux_set = 50
    # Set outputs in TRNData    
